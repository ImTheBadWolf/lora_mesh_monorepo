...
class MessageQueueItem():
  def __init__(self, message_object: Message, snr, last_rebroadcast):
    ...
    self.counter = config.DEFAULT_RESEND_COUNTER if not last_rebroadcast else 1
    self.snr = snr
  ...
  def get_maxhop_from_message(self):
    return self.message[config.HEADER_LENGTH]
  ...
  def decrement_maxhop(self):
    ...
    if not self.last_rebroadcast:
      self.message[config.HEADER_LENGTH] -= 1
    else:
      self.message[config.HEADER_LENGTH] = 0
