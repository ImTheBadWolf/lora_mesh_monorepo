...
class LoRa(object):
    ...
    def send_armachat(self, message):
    ...
    def _handle_interrupt(self, channel):
        ...
        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE):
            ...
            if packet[16] == 33: #or packet[3] == 0xXX (myId)
              #ACK message, or my own message (probably not needed, sent message is not received)
              return

            print("Received packet:")
            print([hex(x) for x in packet])
            stripped_data = packet[16:]
            crypto = AES.new("Sixteen byte key", AES.MODE_CTR, "Sixteen byte key", counter=lambda: b'Sixteen byte key')
            decrypted = crypto.decrypt(bytes(stripped_data))
            msg = ""
            for char in list(decrypted):
              if char != 124:
                msg += chr(char)
